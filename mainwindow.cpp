    ...
    , ui(new Ui::MainWindow)
{
    ...
    map->add_object(200, 400, 60, 5, (int) type_material::GLASS);
    map->add_object(400, 100, 60, 50, (int) type_material::GLASS);
    ...
    map->add_object(400, 300, 60, 5, (int) type_material::DRYWALL);


    map->add_object(500, 600, 60, 5, (int) type_material::IRR_GLASS);
    map->add_object(500, 700, 60, 5, (int) type_material::IRR_GLASS);
...
// диапазоны значений dBm в конкретные цвета
...
//затухание сигнала при прохождении через разные материалы  (алгоритм Брезенхэма)
...
//отрисовка и проверка на наличие материала Она используется для обновления данных на карте, а именно уровня сигнала, через вызов функции signal_atten. Это делается для каждого пикселя, который не содержит материала
void _draw_map(MainWindow *main_win, int start_pos, int step){
    int x, y;
...
void MainWindow::draw(){
    ...
    for(int i = 0; i < COUNT_THREAD; ++i){
        threads[i] = std::thread(_draw_map, this, i, COUNT_THREAD);
...
// для изменения параметра частоты f
...
// визуализации шкалы градиента сигнала
